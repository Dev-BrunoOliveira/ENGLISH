...
export const Auth: React.FC<AuthProps> = ({ onLogin, onSignup, onGoogleLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = isLoginMode ? await onLogin(email, password) : await onSignup(email, password);
  ...
  return (
    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', minHeight: '100vh' }}>
      <div className="glass-panel" style={{ width: '100%', maxWidth: '400px', padding: '3rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', display: 'flex', alignItems: 'center', margin: '2rem 0', gap: '1rem' }}>
        ...
        <button 
          ...
          style={{
            ...
            gap: '12px', fontSize: '1rem', cursor: 'pointer', transition: 'background 0.2s'
          }}
